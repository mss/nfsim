		   ...
		   u8 tos, struct net_device *dev)
{
	...
	log_route(skb, "ERROR: packet is not local and no matching "
		"route (dst=%u.%u.%u.%u)", NIPQUAD(daddr));
...
static struct sk_buff *ip_defrag_user(struct sk_buff *skb, u32 user)
{
	...
	return NULL;
     ...
     LINUX_VERSION_CODE < KERNEL_VERSION(2,4,30))
...
{
	/* Between echo-reply (0) and timestamp (13), everything
	   except echo-request (8) is an error. */
	if (type < ICMP_TIMESTAMP && type!=ICMP_ECHOREPLY && type!=ICMP_ECHO)
		return 1;

	/* Also, anything greater than NR_ICMP_TYPES is unknown, and
	   hence should be treated as an error... */
	return (type > NR_ICMP_TYPES);
}
...
void icmp_send(struct sk_buff *skb_in, int type, int code, u32 info)
{
	...
	if (is_icmp_error(type) && iph->protocol == IPPROTO_ICMP) {
		struct icmphdr ihdr;
