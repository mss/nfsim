...
int skb_ip_make_writable(struct sk_buff **pskb, unsigned int writable_len)
{
	...
	/* We never make packet *less* writable, but beware that packet
	 * could have shrunk ((*pskb)->len less than writable len) due to
	 * NAT mangling.
	 */
	extra = field_value(*pskb, "extra_data");
	if (extra && writable_len < extra->writable_len)
		writable_len = min(extra->writable_len, (*pskb)->len);
...
{
	return dst_path_metric(dst, metric);
}
...
{
	tui_register_pre_post_hook(NULL, run_timers);
}
